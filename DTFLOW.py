    ...
    def plotY(self, size = (20,16),dims=[0,1],color = None,annotation = False):
        ...
        if len(dims) == 2 :
            ...
            if color == None:
                ...
                time_stage = sorted(set(self.stages))  # sorted() 函数对所有可迭代的对象进行排序操作。
                color_stage = cm.jet(np.linspace(0, 1, len(time_stage)))  # matplotlib.cm中的色表
                color_dict = dict(zip(time_stage, color_stage[:len(time_stage)])) 
        ...
        # plt.savefig('/home/wtu/su/code/ourmargarter master-3/figures/PNS/A1.png')
        plt.show()
    def plotY1(self, size = (20,16),dims=[0,1],annotation = False):
        """
        Plot the embedding structure of the data.
        :param color: 'stages','pseudotime','groups'
        """  
        ...
        if len(dims) == 2 :
            ...
            print("The number of dimensions for plots should be 2 .")
    plt.show()
